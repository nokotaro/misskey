 ...
 */
export async function packAvatarEmojis(emojis: string[], ownerHost: string, foreign: boolean): Promise<IRemoteEmoji[]> {
	...
	let avatarEmojis = await Promise.all(avatarKeys.map(async key =>  {
		...
		const profileEmoji: IRemoteEmoji = {
			name: key.emoji,
			url: user.avatarUrl,
			host: key.host,
 ...
 * @param foreign 外部ホスト指定を許可する
 */
export async function packCustomEmojis(emojis: string[], ownerHost: string, foreign: boolean): Promise<IRemoteEmoji[]> {
	...
	let customEmojis = await Promise.all(customKeys.map(async key =>  {
		...
		const customEmoji: IRemoteEmoji = {
			name: key.emoji,
