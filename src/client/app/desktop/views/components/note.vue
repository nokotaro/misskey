		...
		<div class="main">
			...
			<div class="body" v-if="appearNote.deletedAt == null">
				...
				<div class="content" v-show="appearNote.cw == null || showContent">
					...
					<a class="location" v-if="appearNote.geo" :href="`https://maps.google.com/maps?q=${appearNote.geo.coordinates[1]},${appearNote.geo.coordinates[0]}`" rel="noopener" target="_blank"><fa :icon="['fal', 'map-marker-alt']"/> 位置情報</a>
					<div class="renote" v-if="appearNote.renote"><mk-note-preview :note="appearNote.renote"/></div>
					<mk-url-preview v-for="url in urls" :url="url" :key="url" :mini="mini" :compact="compact"/>
				</div>
			</div>
			<footer v-if="!appearNote.deletedAt && !preview" class="footer">
				...
				-->
				<button v-if="['public', 'home', 'followers'].includes(appearNote.visibility)" class="renoteButton button" @click="renote()" :title="$t('renote')">
					<fa :icon="['fal', 'retweet']"/>
					<p class="count" v-if="appearNote.renoteCount">{{ appearNote.renoteCount }}</p>
				</button>
