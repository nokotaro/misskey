...
<mk-ui>
	<template #header>
		<span @click="showNav = true">
			<span :class="$style.title">
				...
				<span v-if="src == 'imas'"><fa icon="share-alt"/>{{ $t('imas') }}</span>
	...
	<main>
		<div class="nav" v-if="showNav">
			...
			<div class="body">
				<div>
					...
					<span :data-active="src == 'imas'" @click="src = 'imas'" v-if="enableLocalTimeline"><fa icon="city"/> {{ $t('imas') }}</span>
		...
		<div class="tl">
			...
			<x-tl v-if="src == 'imas'" ref="tl" key="imas" src="imas"/>
...
export default Vue.extend({
	...
	created() {
		this.$root.getMeta().then((meta: Record<string, any>) => {
			...
			) && ['local', 'hybrid', 'imas'].includes(this.src)) this.src = 'home';
		});
