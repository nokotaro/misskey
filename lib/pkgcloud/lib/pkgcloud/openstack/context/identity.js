	...
	if (self.options.keystoneAuthVersion === 'v3') {
		if (self.options.password) {
			...
			if (self.options.domainId) {
				self._authenticationPayload.auth.identity.password.user.domain = {
					id: self.options.domainId
				};
			} else if (self.options.domainName) {
				self._authenticationPayload.auth.identity.password.user.domain = {
					name: self.options.domainName
				};
			}
			...
			if (self.options.tenantId || self.options.tenantName || self.options.projectDomainName || self.options.projectDomainId) {
				...
				if (self.options.tenantId) {
					self._authenticationPayload.auth.scope.project = {
						id: self.options.tenantId
					};
				} else if (self.options.tenantName) {
					self._authenticationPayload.auth.scope.project = {
						name: self.options.tenantName
					};
				} else {
					scopedProject = false;
				}
				if (!scopedProject) {
					if (self.options.projectDomainId) {
						self._authenticationPayload.auth.scope.domain = {
							id: self.options.projectDomainId
						};
					} else if (self.options.projectDomainName) {
						self._authenticationPayload.auth.scope.domain = {
							name: self.options.projectDomainName
						};
					}
				} else {
					if (self.options.projectDomainId) {
						self._authenticationPayload.auth.scope.project.domain = {
							id: self.options.projectDomainId
						};
					} else if (self.options.projectDomainName) {
						self._authenticationPayload.auth.scope.project.domain = {
...
Identity.prototype._parseIdentityResponse = function (data, headers) {
	...
	if (self.options.keystoneAuthVersion === 'v3') {
		self.token = {
			...
			tenant: {
				id: data.token.project.id,
				name: data.token.project.name
			}
		};
